    ...
    eventsCounts:
      ({ events }) =>
        ...
        // TODO: need to use common mapping with changed ids
          const counts = {
            [EventTypes.VarDump]: events.filter(({ type }) =>
              type === EventTypes.VarDump).length,
            [EventTypes.Smtp]: events.filter(({ type }) =>
              type === EventTypes.Smtp).length,
            [EventTypes.Sentry]: events.filter(({ type }) =>
              type === EventTypes.Sentry).length,
            [EventTypes.Profiler]: events.filter(({ type }) =>
              type === EventTypes.Profiler).length,
            [EventTypes.Monolog]: events.filter(({ type }) =>
              type === EventTypes.Monolog).length,
            [EventTypes.Inspector]: events.filter(({ type }) =>
              type === EventTypes.Inspector).length,
            [EventTypes.HttpDump]: events.filter(({ type }) =>
              type === EventTypes.HttpDump).length,
            [EventTypes.RayDump]: events.filter(({ type }) =>
              type === EventTypes.RayDump).length,
          };
    ...
    cachedIdsTypesList({ cachedIds }) {
      return Object.entries(cachedIds)
        .filter((entry) =>
          entry[1].length > 0)
        .map(([key]) =>
key as PageEventTypes);
    },
    activeProjectKey: ({ projects }) =>
      projects.activeKey,
    activeProject: ({ projects }) => {
      const storedProject = projects.available.find((proj) =>
        proj.key === projects.activeKey);
      const defaultProject = projects.available.find((proj) =>
        proj.is_default);
    ...
    isMultipleProjects: ({ projects }) =>
      projects.available.length > 1 || !projects.available.some((proj) =>
        proj.is_default),
  },
  actions: {
    ...
    initializeEvents(events: ServerEvent<unknown>[]): void {
      ...
      this.events = events
        .filter((el) =>
          availableEvents.includes(el.type as EventType))
        .slice(0, MAX_EVENTS_COUNT);

      this.syncCachedWithActive(events.map(({ uuid }) =>
        uuid));
      this.initActiveProjectKey();
    },
    addList(events: ServerEvent<unknown>[]): void {
      ...
      events
        .filter((el) =>
          availableEvents.includes(el.type as EventType))
        .forEach((event) => {
          const isExistedEvent: boolean = this.events.some((el): boolean =>
            el.uuid === event.uuid);
          if (!isExistedEvent) {
    ...
    removeAll() {
      if (this.lockedIds.length) {
        this.events = this.events.filter(({ uuid }) =>
    ...
    removeByType(eventType: EventType) {
      if (this.lockedIds.length) {
        this.events = this.events.filter(({ type, uuid }) =>
          type !== eventType || this.lockedIds.includes(uuid));

        return;
      }

      this.events = this.events.filter(({ type }) =>
        type !== eventType);
    },
    // cached ids
    ...
    removeLockedIds(eventUuid: EventId) {
      this.lockedIds = this.lockedIds.filter((id) =>
        id !== eventUuid);
    ...
    setAvailableProjects(projects: TProjects["data"]) {
      if (projects.length > 0) {
        ...
        if (!this.projects.activeKey) {
          const defaultProject = projects.find((proj) =>
            proj.is_default) || projects[0];
